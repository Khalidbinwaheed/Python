...
def display_weather(data, units="metric"):
    ...
    try:
        ...
        print(f"Temperature:  {temp:.1f}{temp_unit}" if temp is not None else "Temperature:  N/A")
        print(f"Feels like:   {feels_like:.1f}{temp_unit}" if feels_like is not None else "Feels like:   N/A")
        print(f"Humidity:     {humidity}%" if humidity is not None else "Humidity:     N/A")
        print(f"Pressure:     {pressure} hPa" if pressure is not None else "Pressure:     N/A")
        print(f"Wind Speed:   {wind_speed} {speed_unit}" if wind_speed is not None else "Wind Speed:   N/A")
